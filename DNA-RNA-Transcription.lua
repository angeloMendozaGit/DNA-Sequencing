...
script.Input.Value = ""
	...
	function TranslateToRNA() -- RNA --> Ribonucleic Acid
		...
		for i,v in pairs (BaseSequence) do
			if v == "A" then
				table.insert(ConvertedBaseSequence,"U")
			elseif v == "T" then
...
script.Input.Changed:connect(TranslateToRNA)
